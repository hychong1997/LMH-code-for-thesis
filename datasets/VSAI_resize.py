...
class VSAIDataset_resize(VSAIDataset):
    def __init__(self,
                 pipeline,
                 multi_scales=[(1200, 1200), (400, 400)],
                 use_random_ms=False,
                 **kwargs):
        super(VSAIDataset_resize, self).__init__(pipeline=pipeline, **kwargs)
    ...
    def load_annotations(self, ann_folder):   # ann_folder=ann_file
        ...
        """
        ann_files_path = os.path.join(ann_folder, 'annfiles_distance')
        distance_path = os.path.join(ann_folder, 'distance')
